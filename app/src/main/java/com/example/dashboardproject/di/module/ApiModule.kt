...
class ApiModule(val baseUrl:String) {
    ...
    @Provides
    @Singleton
    fun provideOkHttpClient(logger:HttpInterceptors): OkHttpClient.Builder =
        OkHttpClient.Builder().addInterceptor(logger).addInterceptor(HttpLoggingInterceptor().setLevel(HttpLoggingInterceptor.Level.BODY))
